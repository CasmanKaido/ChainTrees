...
export class QuestGenerator {
   ...
   * @returns {Array} List of quest objects
   */
  generateWeeklyQuests(count = 1) {
                const weeklyTemplates = QUEST_TEMPLATES.filter(t => t.type === QUEST_TYPES.WEEKLY);
                const shuffled = weeklyTemplates.sort(() => 0.5 - Math.random());
                const selected = shuffled.slice(0, count);

                return selected.map(template => ({
                    ...template,
                    instanceId: `weekly_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
                    progress: 0,
                    completed: false,
                    claimed: false,
                    startDate: new Date().toISOString(),
                    expiresAt: this.getEndOfWeek().toISOString()
                }));
            }
  ...
  /**
   * Get date object for end of current week (Sunday)
   */
  getEndOfWeek() {
                const date = new Date();
