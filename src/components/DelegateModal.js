...
export class DelegateModal {
  ...
  createModal() {
    ...
    // Listeners
    modal
      .querySelector('#submit-delegate-btn')
      .addEventListener('click', () => this.handleDelegate())
  ...
  open(userAddress) {
    ...
    if (currentDelegate !== userAddress) {
      document.getElementById('delegate-status').textContent =
        `Delegated to ${currentDelegate.substr(0, 10)}...`
